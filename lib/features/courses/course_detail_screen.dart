...
class _CourseDetailScreenState
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context)
          ...
          const SnackBar(
            content:
                Text('Could not enroll right now.'),
          ),
        );
      }
    } finally {
  ...
  Future<void> _showTopUpDialog(
    ...
    final mtnNumber =
        details?['momo_wallet_number']
                ?.toString() ??
            '';

    final airtelNumber =
        details?['airtel_merchant_number']
                ?.toString() ??
            '';

    final shortfall =
        details?['shortfall']?.toString() ??
            '';
                      ...
                      children: [
                        ...
                        Text(
                          activeNumber.isEmpty
                              ? 'Send at least \$$shortfall (USD) using $activeLabel, then enter the transaction reference below. An admin will review and credit your wallet.'
                              : 'Send at least \$$shortfall (USD) to $activeNumber using $activeLabel, then enter the transaction reference below. An admin will review and credit your wallet.',
                        ),
                      ],
                    ),
                  ),
                ...
                onPressed: () async {
                  ...
                  if (amount <= 0 ||
                    ...
                    ScaffoldMessenger.of(
                      ...
                      const SnackBar(
                        content: Text(
                          'Enter a valid amount and payment reference.',
                        ),
                      ),
                    );
                    return;
    ...
    return RefreshIndicator(
      ...
      child: ListView(
        ...
        children: [
          ...
          Text(
            'Curriculum · ${course.totalLessons} lessons',
            style:
                const TextStyle(
              fontSize: 17,
              fontWeight:
    ...
    try {
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(
          ...
          const SnackBar(
            content: Text(
              'Enrolled — try opening the file again.',
            ),
          ),
        );
      }
          ...
          return Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                // A lesson can have a linked
                // video URL, an attached file,
                // both, or neither.
                OutlinedButton.icon(
                  onPressed: () async {
                    final result =
